...
import be.kdg.keepdishesgoing.restaurant.adapter.out.RestaurantMapper;
...
public class RestaurantController {
    ...
    public ResponseEntity<RestaurantDto> createRestaurant(@RequestBody CreateRestaurantRequest request) throws OwnerAlreadyHasRestaurantException {
        ...
        List<OpeningHours> openingHours = request.openingHours().stream()
                .map(ohr -> new OpeningHours(
                        ohr.dayOfWeek(),
                        ohr.openingTime(),
                        ohr.closingTime())
                ).toList();
        CreateRestaurantCommand command = new CreateRestaurantCommand(request.ownerId(), request.name(),
                address, request.email(), request.pictures(), request.cuisineType(),
                request.preparationTime(), openingHours);
