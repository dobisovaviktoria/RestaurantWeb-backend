# Keep Dishes Going

This is the backend for Keep Dishes Going application. In order to sucessfully run the website, this backend needs to run first.

## Author 
Viktória Dobišová

## Finished Features

List all features that were successfully implemented and tested.

- [x] Sign in/sign up
- [x] Create restaurant
- [x] Editing drafts without affecting live menu
- [x] Publish/unpublish dish
- [x] Applying all pending dish changes at once
- [x] Marking dish in/out of stock
- [x] Accepting/rejecting orders
- [x] Marking accepted order for ready to pickup
- [x] Landing page and continuing as a customer
- [x] Customers having list of restaurants to explore
- [x] Viewing restaurant's dishes
- [x] Building a basket from a single restaurant
- [x] Customer checkout information form
- [x] Each owner manages exactly 1 restaurant
- [x] Automatically decline order after 5 minutes
- [x] Max 10 dishes available for a restaurant
- [x] Publishing messages for delivery (MQ)
- [x] Ordering without signing up
- [ ] Guesstimated delivery time
- [ ] Stopping basket from checkout if dish becomes unavailable
- [ ] Payment provider
- [ ] Confirmation link to track an order
- [ ] Price range evolution 
- [ ] Adjusting price range or price range in general
